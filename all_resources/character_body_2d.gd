...
func _physics_process(delta):
	...
	if global.phealth<=0:
		...
		player_alive = false
		global.phealth=0
		var current_t = global.current_time
		$AnimatedSprite2D.play('death')
		await get_tree().create_timer(1).timeout
		if(get_tree().current_scene.name == 'world'):
			global.BT1 = 0
		if(get_tree().current_scene.name == 'another_world'):
			global.BT2 = 0
		if(get_tree().current_scene.name == 'Node2D'):
			global.BT3 = 0
		get_tree().reload_current_scene()
		global.current_time = current_t
		global.phealth=100
...
func enemy_attack():
	if enemy_in_attack_range==true and enemy_attack_cooldown==true :
		if(global.last_boss):
			global.phealth = global.phealth - 30
		else:
...
func _on_regen_timer_timeout() -> void:
	if global.phealth<200:
		global.phealth = global.phealth+10
		if global.phealth>200:
			global.phealth=200
	if global.phealth<=0:
		global.phealth=0
